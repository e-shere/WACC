...
class MatchOutputTests extends AnyFlatSpec {
  ...
  "exitBasic program" should "exit with code 7" in {
    parse(new File("src/examples/valid/basic/exit/exitBasic.wacc"))
      .toString() shouldBe "Success(WaccProgram(List(),List(Exit(IntLiter(7)(12,8))(12,3)))(12,3))"
  }

  "exit-1 program" should "exit with code -1" in {
    parse(new File("src/examples/valid/basic/exit/exit-1.wacc"))
      .toString() shouldBe "Success(WaccProgram(List(),List(Exit(IntLiter(-1)(12,8))(12,3)))(12,3))"
  }

  "commentInLine program" should "only skip" in {
    parse(new File("src/examples/valid/basic/skip/commentInLine.wacc"))
      .toString() shouldBe "Success(WaccProgram(List(),List(Skip()(9,3)))(9,3))"
  }

  "ifBasic program" should "skip in both branches" in {
    parse(new File("src/examples/valid/if/ifBasic.wacc"))
      .toString() shouldBe "Success(WaccProgram(List(),List(If(BoolLiter(true)(9,6),List(Skip()(11,5)),List(Skip()(13,5)))(9,3)))(9,3))"
  }

  "functionDeclaration program" should "declare the 0 function" in {
    parse(
      new File(
        "src/examples/valid/function/simple_functions/functionDeclaration.wacc"
      )
    ).toString() shouldBe
  ...
  "booleanExprs program" should "use and, or and not" in {
    parse(new File("src/examples/outputTests/boolExprs.wacc"))
      .toString() shouldBe
